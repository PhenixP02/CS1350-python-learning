...
match2 = re.search(r"[A-H][1-9]", text)
if match2:
...
match3 = re.search(r"s[1-9][1-9][1-9][1-9][1-9]", text)
if match3:
...
for text in samples:
    ...
    # Hint:
    digit_lower = re.search(r"\d[a-z]", text)  ## NO PATTERN MATCHES IN GIVEN SAMPLES
    if digit_lower:
...
for line in log_lines:
    ...
    is_error = bool(re.search(r"ERROR", line))

    # TODO 3: Search for a 4-digit number (digit NOT preceded/followed by dash)
    port_match = re.search(r"(?<!-)\d\d\d\d(?!-)", line)
    # Build output
    if date_match:
